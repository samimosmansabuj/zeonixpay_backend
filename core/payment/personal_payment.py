...
class PersonalAgentPaymentBaseView(views.APIView):
    ...
    def _verify_payment(self, transaction_id: str, invoice):
        ...
        msg = fetch_one(
            StorePaymentMessage.objects.filter(src_q, trx_id=transaction_id, message_amount=invoice.customer_amount)
        )
        ...
        msg.save(update_fields=["is_verified"])
        return True
...
#                 verify = self.verify_payment(transaction_Id, invoice)
...
#                         "Number": f"{payment_gateway.details_json['phone_number'] if payment_gateway is not None else 'Bkash Cashout Method Not Available Right Now, Try Another Method!'}",
...
#                     "data": {
#                         "Method": f"{payment_method.title()}",
#                         "Number": f"{payment_gateway.details_json['phone_number'] if payment_gateway is not None else 'Bkash Send Money Method Not Available Right Now, Try Another Method!'}",
#                         "Message": f"Submit Your Phone Number & Transaction ID. Cashout Amount is {invoice.customer_amount}"
#                     }
#                     # 'invoice': InvoiceSerializer(invoice).data
#                 }, status=status.HTTP_200_OK
...
#     def verify_payment(self, transaction_Id, invoice):
#         if StorePaymentMessage.objects.filter(message_amount=invoice.customer_amount, trx_id=transaction_Id, message_from__icontains="bKash").exists():
...
#                 return True
#         elif StorePaymentMessage.objects.filter(message__icontains=str(transaction_Id), message_from__icontains="bKash").exists():
#             bkash_payment_messages = StorePaymentMessage.objects.filter(message__icontains=str(transaction_Id))
#             if len(bkash_payment_messages) != 1:
#                 raise ValidationError("Exist Multiple Object With this Transaction ID, Please contact support team!")
#             bkash_payment_messages = bkash_payment_messages.first()
#             if bkash_payment_messages.is_verified == True:
#                 raise ValidationError("With this Transaction ID and Amount is already verified, Try again!")
#             else:
#                 bkash_payment_messages.is_verified = True
...
#                     "data": {
#                         "Method": f"{payment_method.title()}",
#                         "Number": f"{payment_gateway.details_json['phone_number'] if payment_gateway is not None else 'Nagad Cashout Method Not Available Right Now, Try Another Method!'}",
#                         "Message": f"Submit Your Phone Number & Transaction ID. Cashout Amount is {invoice.customer_amount}"
#                     }
#                 }, status=status.HTTP_200_OK
#             )
...
#                     "data": {
#                         "Method": f"{payment_method.title()}",
#                         "Number": f"{payment_gateway.details_json['phone_number'] if payment_gateway is not None else 'Nagad Send Money Method Not Available Right Now, Try Another Method!'}",
...
#     def verify_payment(self, transaction_Id, invoice):
#         if StorePaymentMessage.objects.filter(message_amount=invoice.customer_amount, trx_id=transaction_Id, message_from__icontains="NAGAD").exists():
#             nagad_payment_messages = get_object_or_404(StorePaymentMessage, message_amount=invoice.customer_amount, trx_id=transaction_Id)
#             if nagad_payment_messages.is_verified == True:
#                 raise ValidationError("With this Transaction ID and Amount is already verified, Try again!")
#             else:
#                 nagad_payment_messages.is_verified = True
#                 nagad_payment_messages.save()
#                 return True
#         elif StorePaymentMessage.objects.filter(message__icontains=str(transaction_Id), message_from__icontains="NAGAD").exists():
#             nagad_payment_messages = StorePaymentMessage.objects.filter(message__icontains=str(transaction_Id))
#             if len(nagad_payment_messages) != 1:
#                 raise ValidationError("Exist Multiple Object With this Transaction ID, Please contact support team!")
#             nagad_payment_messages = nagad_payment_messages.first()
#             if nagad_payment_messages.is_verified == True:
#                 raise ValidationError("With this Transaction ID and Amount is already verified, Try again!")
#             else:
#                 nagad_payment_messages.is_verified = True
...
#                     "data": {
#                         "Method": f"{payment_method.title()}",
#                         "Number": f"{payment_gateway.details_json['phone_number'] if payment_gateway is not None else 'Rocket Cashout Method Not Available Right Now, Try Another Method!'}",
#                         "Message": f"Submit Your Phone Number & Transaction ID. Cashout Amount is {invoice.customer_amount}"
#                     }
#                     # 'invoice': InvoiceSerializer(invoice).data
#                 }, status=status.HTTP_200_OK
...
#                     "data": {
#                         "Method": f"{payment_method.title()}",
#                         "Number": f"{payment_gateway.details_json['phone_number'] if payment_gateway is not None else 'Rocket Send Money Method Not Available Right Now, Try Another Method!'}",
#                         "Message": f"Submit Your Phone Number & Transaction ID. Send Money Amount is {invoice.customer_amount}"
#                     }
#                 }, status=status.HTTP_200_OK
#             )
...
#     def verify_payment(self, transaction_Id, invoice):
#         if StorePaymentMessage.objects.filter(message_amount=invoice.customer_amount, trx_id=transaction_Id, message_from__icontains="16216").exists():
#             rocket_payment_messages = get_object_or_404(StorePaymentMessage, message_amount=invoice.customer_amount, trx_id=transaction_Id)
#             if rocket_payment_messages.is_verified == True:
#                 raise ValidationError("With this Transaction ID and Amount is already verified, Try again!")
#             else:
#                 rocket_payment_messages.is_verified = True
#                 rocket_payment_messages.save()
#                 return True
#         elif StorePaymentMessage.objects.filter(message__icontains=str(transaction_Id), message_from__icontains="16216").exists():
#             rocket_payment_messages = StorePaymentMessage.objects.filter(message__icontains=str(transaction_Id))
#             if len(rocket_payment_messages) != 1:
#                 raise ValidationError("Exist Multiple Object With this Transaction ID, Please contact support team!")
#             rocket_payment_messages = rocket_payment_messages.first()
#             if rocket_payment_messages.is_verified == True:
#                 raise ValidationError("With this Transaction ID and Amount is already verified, Try again!")
#             else:
#                 rocket_payment_messages.is_verified = True
